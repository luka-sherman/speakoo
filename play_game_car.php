    ...
    <body>
        ...
        <div class="container-fluid">
            ...
            <script>
                ...
                var final_string = JSON.parse(JSON.stringify(fsarray));
                ...
                function populatePopup() {
                    ...
                    qstring += '</span>';
                    toDisplay += qstring + "</br>";

                    // radio button for the textbox?
                    for (i = 0; i < optionLength; i++) {
                        toDisplay = toDisplay + '<input type = "radio" name = "answer" id="option' + i + '" value = ' + optionarray[i];
                        if (i == 0) {
                            toDisplay += ' checked = "checked"'
                        }
                        toDisplay += '> <label for="option' + i + '">' + optionarray[i] + '</label> </br>';

                    }

                    toDisplay += '<input type="radio" name="answer" id="delete" value="$$$"><label for = "delete"> Delete ' + origverb + " </label> </br>"
                    toDisplay += '...Or type it yourself </br><input type = "text" name = "answer" id="modify"><label for = "modify"> </label> </br>'
                ...
                function popDown() {
                    ...
                    paused = false;
                    //fsarray[currentWord] = document.forms[0].answer.value;
                    final_string[(wordsArray[currentWord - 1].word_index) - 1] = document.forms[0].answer.value; //currentWord goes like 1,2,3,4



                    //console.log(final_string.join(" ").replace("$$$","").replace("  "," "));
                    //console.log(final_string.join(" ").split('$$$').join(' ').replace(/ +(?= )/g,''));
                    if (currentWord >= wordsArray.length) {
                        var $corrected_sentence_0 = final_string.join(" ").split('$$$').join(' ').replace(/ +(?= )/g, '');
                        console.log($corrected_sentence_0);
                        var $pass_task_id = taskID;
                        console.log($pass_task_id);
                        var n = fullSentence.localeCompare($corrected_sentence_0);
                        if (n == 0) {
                        ...
                        timeoutVar = window.setTimeout(displayPopup, 5000);
                    }
                }
            </script>
            <script>
