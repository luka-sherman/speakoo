...
//----------------------------------------------------
  ...
  //$test="/usr/local/bin/ffmpeg -i uploads/$dataKey.wav -itsoffset -00:00:00 -i uploads/$dataKey.webm -map 0:0 -map 1:0 uploads/$dataKey-merge.webm";
...
//----------------------------------------------------
  ...
  $audioVideoDir = "C:/inetpub/wwwroot/RocLang/uploads/";
  //$exeDir = "C:/inetpub/wwwroot/Release_standalone/Audio/";
  $exeDir = "C:/inetpub/wwwroot/RocLang/AVProcessor_2015_mar/AVProcessor/bin/Release/";
  ...
  if (file_exists($audioVideoFeaturesFile) and file_exists($averageFeaturesFile)) {
    ...
    if ($_GET["private"] == "true") {
      ...
      if (file_exists($audioAlignmentFile)) {
        $_SESSION["$dataKey-wordAudioAlignmentData"] = file_get_contents($audioAlignmentFile);
        unlink($audioAlignmentFile);
      }
      if (file_exists($wordProsodyFile)) {
        $_SESSION["$dataKey-wordProsodyAudioData"] = file_get_contents($wordProsodyFile);
        unlink($wordProsodyFile);
      }
	    if (file_exists($transcriptGoogleFile)) {
        unlink($transcriptGoogleFile);
      }
      if (file_exists($transcriptNuanceFile)) {
        unlink($transcriptNuanceFile);
      }
	    if (file_exists($formattedTranscriptFile)) {
        unlink($formattedTranscriptFile);
      }
      /*
      if (file_exists($videoMillisFile)) {
        unlink($videoMillisFile);
      }
      */
      if (file_exists($voicedAudioIntervalFeatureFile)) {
        $_SESSION["$dataKey-voicedAudioIntervalFeatureData"] = file_get_contents($voicedAudioIntervalFeatureFile);
        unlink($voicedAudioIntervalFeatureFile);
      }
